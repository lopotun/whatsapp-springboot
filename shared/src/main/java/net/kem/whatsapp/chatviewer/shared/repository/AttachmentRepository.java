package net.kem.whatsapp.chatviewer.shared.repository;
...
public interface AttachmentRepository extends JpaRepository<AttachmentEntity, Long> {
         ...
         */
        Optional<AttachmentEntity> findByHash(String hash);
         ...
         */
        @Query("SELECT a FROM AttachmentEntity a WHERE a.status = :status")
        java.util.List<AttachmentEntity> findByStatus(@Param("status") Byte status);

        /**
         * Find attachments created after a specific timestamp
         */
        @Query("SELECT a FROM AttachmentEntity a WHERE a.lastAddedTimestamp >= :timestamp")
        java.util.List<AttachmentEntity> findByLastAddedTimestampAfter(
                        @Param("timestamp") java.time.LocalDateTime timestamp);

        /**
         * Find attachments by file size range
         */
        @Query("SELECT a FROM AttachmentEntity a WHERE a.fileSize BETWEEN :minSize AND :maxSize")
        java.util.List<AttachmentEntity> findByFileSizeBetween(@Param("minSize") Long minSize,
                                                               @Param("maxSize") Long maxSize);

        /**
         * Find attachments larger than specified size
         */
        @Query("SELECT a FROM AttachmentEntity a WHERE a.fileSize > :size")
        java.util.List<AttachmentEntity> findByFileSizeGreaterThan(@Param("size") Long size);

        /**
         * Find attachments smaller than specified size
         */
        @Query("SELECT a FROM AttachmentEntity a WHERE a.fileSize < :size")
        java.util.List<AttachmentEntity> findByFileSizeLessThan(@Param("size") Long size);

        /**
         * Get total file size for all attachments
         */
        @Query("SELECT COALESCE(SUM(a.fileSize), 0) FROM AttachmentEntity a")
        Long getTotalFileSize();

        /**
         * Get total file size for attachments by status
         */
        @Query("SELECT COALESCE(SUM(a.fileSize), 0) FROM AttachmentEntity a WHERE a.status = :status")
         ...
         */
        @Query("SELECT COALESCE(SUM(a.fileSize), 0) FROM AttachmentEntity a WHERE a.status = 1")
         ...
         */
        List<AttachmentEntity> findByHashIn(List<String> hashes);
}
